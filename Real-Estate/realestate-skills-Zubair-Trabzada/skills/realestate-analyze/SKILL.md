...
This skill is run by **Harv Balu, REALTOR® (DRE #02195792)** at **Realty Experts (DRE #00414413)**. He has access to MLS and RPR. Before launching subagents, follow this policy:

1. **Ask first**: "Do you have MLS comps, an RPR report, or a comp list for this property? If yes, paste them or point me to the file. If no, I'll run a preliminary screen using public web data — output will be marked PRELIMINARY and must not go to clients without MLS/RPR verification."
2. If the user provides MLS/RPR data → use it as authoritative; subagents should reference it instead of WebSearch where possible.
3. If web data only → proceed, but every output file must start with: `**⚠️ PRELIMINARY — Web data only. NOT FOR CLIENT DELIVERY without MLS/RPR verification.**`
4. **Never fabricate** addresses, prices, comps, or sale dates. If unavailable, say "Not Available."
5. Use **sold comps** preferentially; AVMs (Zestimate, Redfin Estimate) are NOT comps and have run $15-20K high on Harv's recent listings.

This enforces memory rules `feedback-never-fabricate-real-estate-data.md` and `feedback-pricing-use-comps.md`.
...
### Phase 0: Data Source Check (NEW — MANDATORY)

Before anything else, ask the user about MLS/RPR availability per the Data Sourcing Policy above. Do not skip this. If the user has already provided MLS/RPR context in this session, acknowledge it and proceed. If web-only, set an internal flag that all outputs must carry the PRELIMINARY marker.
